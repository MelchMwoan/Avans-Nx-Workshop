...
export class TrainingService {
  ...
  async getOne(identifier: string): Promise<ITraining> {
    ...
    const training = await this.trainingModel
      .findOne(isObjectId ? { _id: identifier } : { name: identifier })
      .exec();
    if (!training) {
  ...
  async delete(identifier: string, req: any): Promise<void> {
    ...
    try {
      ...
      if (!training)
        throw new NotFoundException(`Training could not be found!`);
      if (!training.trainers.includes(req['user'].sub))
        throw new UnauthorizedException('You are not the owner of this entity');
      const result = await this.trainingModel
        .deleteOne({ name: identifier })
        .exec();
      const res = await this.neo4jService.write(
        'MATCH (t:Training {name: $trainingName}) OPTIONAL MATCH (t)-[r]-() DELETE t,r',
        {
  ...
  async create(training: CreateTrainingDto, req: any): Promise<ITraining> {
    ...
    const existingTraining = await this.trainingModel.findOne({
      name: training.name,
    });
    if (existingTraining) {
    ...
    await this.validateReferencesExist(
      [training.roomId],
      'Room not found',
      this.roomModel
    );
    ...
    await this.validateReferencesExist(
      training.trainers,
      'One or more trainers not found',
      this.userModel
    );
    try {
      const newTraining = {
      ...
      if (training.trainers == null) {
        ...
        newTraining.trainers.indexOf(req.user.sub) == -1
        ...
        'MERGE (t:Training {name: $trainingName}) ON CREATE SET t.id = $trainingId, t.difficulty = $trainingDifficulty WITH t MATCH (ro:Room {id:$roomId}) MERGE (t)-[y:TAKES_PLACE_IN]->(ro) WITH t MATCH (u:User) WHERE u.id IN $trainingTrainers MERGE (u)-[r:HOST]->(t) WITH t MATCH (e:Exercise) WHERE e.id IN $trainingExercises MERGE (e)-[x:CONTAINS]->(t)',
         {
          ...
          trainingTrainers: createdTraining.trainers.map((objectId) =>
            String(objectId)
          ),
  ...
  async update(
    ...
    try {
      ...
      if (!existingTraining.trainers.includes(req['user'].sub))
        throw new UnauthorizedException('You are not an owner of this entity');

      if (training.roomId != null)
        await this.validateReferencesExist(
          [training.roomId],
          'Room not found',
          this.roomModel
      ...
      const res = await this.neo4jService.write(
        'MATCH(t:Training {name:$trainingOldName}) SET t.name = $trainingNewName, t.difficulty = $trainingNewDifficulty',
        {
  ...
  async join(identifier: string, req: any): Promise<IEnrollment> {
    ...
    if (enrollment)
      throw new ConflictException('You already joined this training.');
    try {
      const newEnrollment = {
        training: training,
        player: player,
      ...
      const res = await this.neo4jService.write(
        'MATCH (u:User {email: $playerEmail}) MATCH (t:Training {name: $trainingName}) MERGE (u)-[role:PLAYED]->(t)',
        {
  ...
  async leave(identifier: string, req: any): Promise<void> {
    ...
    if (!enrollment)
      throw new NotFoundException(`Enrollment could not be found!`);
    try {
      const result = await this.enrollmentModel
        .deleteOne({ _id: enrollment._id })
        .exec();
      const res = await this.neo4jService.write(
        'MATCH (u:User {email: $userEmail}) -[r:PLAYED]->(t:Training {name: $trainingName}) DELETE r',
        {
  ...
  async getEnrollmentsByTraining(
    identifier: string,
    req: any
  ): Promise<IEnrollment[]> {
