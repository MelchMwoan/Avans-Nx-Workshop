...
import { Difficulty, IExercise, IPlayer, IRoom, ITrainer, ITraining, IUser } from '@avans-nx-workshop/shared/api';
...
export class TrainingEditComponent implements OnInit, OnDestroy {
  ...
  constructor(private trainingService: TrainingService, private formBuilder: FormBuilder, private router: Router, private route: ActivatedRoute, private authService: AuthService, private alertService: AlertService, private userService: UserService, private exerciseService: ExerciseService, private roomService: RoomService) {}
  ...
  ngOnInit(): void {
    ...
    this.routeSub = this.route.params.subscribe(async params => {
      ...
      if(params['id']) {
        ...
        document.querySelector('#deleteTrainingBtn')?.addEventListener('click', function() {modal.show()});
  ...
  onSubmit() {
    ...
    if(this.createTrainingForm.invalid) return this.createTrainingForm.markAllAsTouched();
    if(this.training != null) {
      console.log(`Updating: ${this.createTrainingForm}`);
      const id = this.training.name;
        const training: UpdateTrainingDto = this.createTrainingForm.value as unknown as UpdateTrainingDto;
        this.subscription = this.trainingService.update(id, training).subscribe((results) => {
          console.log(results);
          this.router.navigate(['/training/'+results.results.name])
        });
    } else {
        console.log(`Creating training: ${JSON.stringify(this.createTrainingForm.value)}`);
        let training: CreateTrainingDto = this.createTrainingForm.value as unknown as CreateTrainingDto;
        training = {
          ...training,
          room: training.roomId
        }
        this.subscription = this.trainingService.create(training).subscribe((results) => {
          this.router.navigate(['/training/'+results.results.name])
        });
      }
  }
  ...
  deleteTraining() {
    ...
    this.subscription = this.trainingService.delete(trainingId).subscribe((results) => {
      this.router.navigate(['/trainings'])
    });
  }
}
